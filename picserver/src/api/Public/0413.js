        ...
        AddCases:function (){
            ...
            ep.once('getPolicyno',function(result){
                ...
                for(var i=0;i<result.length;i++){
                    ...
                    if(result[i].insurant==insurant&&result[i].injuredCondition==injuredCondition){
                        flag=true;
                        break;
                    }
                }
...
//                return Me.cb(200,null,storageID);
       ...
       fcxQueryCaseInfo:function(){
          ...
          var sqlCmd = "";
          var sqlParams = [];
          var policy_no =Me.getParam("plug_policyNo"); //保单号查询
          ...
          var outEndTime=Me.getParam("plug_outEndTime"); //出险结束时间
             sqlCmd = "select * from cases " +
                "where case_state = 0";
            if (policy_no) {
                sqlWhere += ' and policyNo like "%' + policy_no + '%"';
            }
